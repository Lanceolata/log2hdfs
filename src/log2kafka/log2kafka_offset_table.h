...
#ifndef LOG2HDFS_LOG2KAFKA_LOG2KAFKA_OFFSET_TABLE_H_
#define LOG2HDFS_LOG2KAFKA_LOG2KAFKA_OFFSET_TABLE_H_
...
class Log2kafkaOffsetTable {
 public:
  static std::shared_ptr<Log2kafkaOffsetTable> Init(
      const std::string& path, int interval);

  Log2kafkaOffsetTable(const std::string& path, int interval):
      path_(path), interval_(interval) {
    if (IsFile(path))
      Remedy();
  }

  ~Log2kafkaOffsetTable() {
    Stop();
  }

  Log2kafkaOffsetTable(const Log2kafkaOffsetTable& other) = delete;
  Log2kafkaOffsetTable& operator=(const Log2kafkaOffsetTable& other) = delete;
...
#endif  // LOG2HDFS_LOG2KAFKA_LOG2KAFKA_OFFSET_TABLE_H_
